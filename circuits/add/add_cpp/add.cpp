...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 4],&signalValues[mySignalStart + 3]); // line circom 12
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_eq(&expaux[0],&signalValues[mySignalStart + 0],&circuitConstants[0]); // line circom 13
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 13. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
